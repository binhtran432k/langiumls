...
export class LangiumGrammarWorkspaceManager extends DefaultWorkspaceManager {
	...
	override async initializeWorkspace(
		...
		const buildConf: WorkspaceManagerConf =
			await this.configurationProvider.getConfiguration("langium", CONFIG_KEY);
		const ignorePatterns = buildConf.ignorePatterns
			?.split(",")
			?.map((pattern) => pattern.trim())
			?.filter((pattern) => pattern.length > 0);
		this.matcher = ignorePatterns
			? ignore.default().add(ignorePatterns)
			: undefined;
	...
	protected override includeEntry(
		...
		if (this.matcher) {
			...
			return (
				...
				(entry.isDirectory ||
					(entry.isFile &&
						fileExtensions.includes(UriUtils.extname(entry.uri))))
			);
		}
